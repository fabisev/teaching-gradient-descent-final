...
let fileURLToPath;
try { ({ fileURLToPath } = await import('node:url')); } catch {}
let nodeCrypto;
try { nodeCrypto = await import('node:crypto'); } catch {}
const PLUGIN_PATH = fileURLToPath ? fileURLToPath(import.meta.url) : new URL(import.meta.url).pathname;
...
function createWidgetId() {
  return nodeCrypto?.randomUUID?.() || globalThis.crypto?.randomUUID?.() || `lightbox-${Math.random().toString(36).slice(2)}`;
}
